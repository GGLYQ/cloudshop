...
import {  AddressList ,ContactCard ,TreeSelect ,Divider,SubmitBar,Stepper,SwipeCell,Checkbox, CheckboxGroup,Card,ActionBar, ActionBarIcon, ActionBarButton,Tabbar, TabbarItem,Swipe, SwipeItem,Search,Icon ,Button,Form, Field, CellGroup,NavBar } from 'vant';
import 'vant/lib/index.css';

createApp(App).use(createPinia()).use(AddressList).use(ContactCard).use(TreeSelect).use(Divider).use(SubmitBar).use(Stepper).use(SwipeCell).use(Checkbox).use(CheckboxGroup).use(Card).use(ActionBar).use(ActionBarIcon).use(ActionBarButton).use(Tabbar).use(TabbarItem).use(Swipe).use(SwipeItem).use(Search).use(NavBar).use(Form).use(Field).use(CellGroup).use(Icon).use(Button).use(router).mount('#app')
